import { Master } from './sharder/Sharder';
import BotSchema from './mongo/models/Bot';
...
export default class Client {
    public constructor(token: string) {
        new Connection(process.env.MONGO_URL);

        _client = new Master(token, {
            clientOptions: {
                restMode: true,
                messageLimit: 100,
                autoreconnect: true,
        ...
        _client.on("clusters", async (data: any) => {
            await BotSchema.findOneAndUpdate({ id: bot_schema_model }, { clusters: data });
        });
    }
}

export const MyClient: any = _client;