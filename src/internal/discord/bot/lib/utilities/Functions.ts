...
export default class Functions {
    ...
    protected auto_reboot: any;

    constructor(client: any, key: any, options?: any) {
        if(!client) throw Error("Undefined client.");
        this.client = client;
        if(!key || (key && key !== "jean")) throw Error("Unathorized");
    ...
    */
    public protocolZ(key: any): void {
        ...
        if(!"jean".includes(key)) throw Error("Unauthorized.")
        process.exit();
    }
    ...
    */
    public format(timestamp: number, lang: number, options: any) {
        if(!timestamp) throw Error("Unknown timestamp.");
        let localLang: any = formatLangs[lang];
    ...
    */
    public resolveUser(list: any, argument: any) {
        ...
        if (idMatch) {
            ...
            if (list.find((e: any) => e.discriminator && e.username) &&argument.length > 5 && argument.slice(-5, -4) === '#') {
                member = list.find((e: any) => (`${e.username}#${e.discriminator}`.toLowerCase()).startsWith(argument.toLowerCase()))
            } else if(list.find((e: any) => e.discriminator) && argument.length < 5 && !isNaN(argument)) {
                member = list.find((e: any) => `${e.discriminator}` === argument)
            } else if(list.find((e: any) => e.discriminator) && argument.startsWith("#") && argument.length === 5 && !isNaN((argument.slice(1)))) {
                member = list.find((e: any) => `#${e.discriminator}` === argument)
            } else {
                member = list.find((e: any) => e.username.toLowerCase().startsWith(argument.toLowerCase()))
            }
        }
    ...
    public getMemberID(argument: any) {
        const idMatcher = /^([0-9]{15,21})$/;
        const userMentionMatcher = /<@!?([0-9]{15,21})>/;

        const idMatch = idMatcher.exec(argument) || userMentionMatcher.exec(argument)[1];
    ...
    */
    public randomKey(length: number) {
